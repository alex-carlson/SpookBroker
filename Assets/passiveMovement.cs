...
public enum MovementType { Floating, Shaking, Spinning, SpinFloat, SineRotate };
...
public class passiveMovement : MonoBehaviour {
    ...
    void Update () {
		if (myMotion == MovementType.Floating) {
			...
			Shaking ();
		} if (myMotion == MovementType.SineRotate) {
			SineRotate ();
		}
	}
	...
	void SineRotate(){
		...
		transform.rotation = Quaternion.Euler (new Vector3(0, 0, Mathf.Sin( Time.time ) * intensity));
	}
}
